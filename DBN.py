...
if __name__ == '__main__':
    ...
    new_training = np.asarray(r.run_visible(training_data[-1]))
    #print(r.weights)
    d = DBN(num_visible=100, num_hidden=7,weights=r.weights)
    d.train(new_training, max_epochs=10000)
    #layer_three = np.asarray(d.run_visible(new_training[-1]))
    #three = DBN(num_hidden=7, num_visible=2,weights=d.weights)
