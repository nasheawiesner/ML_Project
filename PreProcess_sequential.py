                ...
                for j in i['pointsJson']: #iterate through events
                    if j["events"][0]["type"] != "Cessation" and 'line' in j:
                        ...
                        for k in j['events']:  #iterate through actions
                            ...
                            if k['type'] == "Offense":
                                # if this is the first pass, add the thrower to event list
                                if len(event_list) == 0:
                                    event_list.append(k['passer'])
                                # add receiver to event list, unless receiver is 'anonymous'
                                if k['receiver'] != 'Anonymous':
                                    event_list.append(k['receiver'])
                                numPasses += 1
                                if k["action"] == "Goal":
                                    scored = 1
